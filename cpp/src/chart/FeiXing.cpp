#include "chart/Feixing.h"
#include <cstring> // for strncpy

FeixingChart::FeixingChart(int year, int lunarMonth, int lunarDay, int timeBranch, bool isMale) {
    fullChart = new Chart(year, lunarMonth, lunarDay, timeBranch, isMale);
}

FeixingChart::~FeixingChart() {
    delete fullChart;
}

Palace FeixingChart::getPalace(int index) const {
